...
const jElijahDetailHtml = engine.playerDetailHtml('j-elijah-lt', 'rutgers', 'LT');
check('Verified Rutgers player attributes surface on player detail overview and attributes tabs', ['J. Elijah','Acceleration','82','Strength','80','Awareness','74'].every(token => jElijahDetailHtml.includes(token)) && jElijahDetailHtml.includes('premium-player-profile') && jElijahDetailHtml.includes('N/A'));
check('Player detail highlights verified data and collapses optional source-missing profile fields', jElijahDetailHtml.includes('Verified data available') && jElijahDetailHtml.includes('source-missing-note') && jElijahDetailHtml.includes('Source fields not shown') && !jElijahDetailHtml.includes('Height</span><strong>N/A'));
...
const earlyStatsWorkspaceHtml = engine.renderStatsWorkspace();
check('Stats Hub renders structured sports stat tables without raw object previews', earlyStatsWorkspaceHtml.includes('sports-stat-table') && !earlyStatsWorkspaceHtml.includes('Team Snapshot') && !/schema version|package type|source video|verification status|\[object Object\]|name:|player id:/i.test(earlyStatsWorkspaceHtml));
...
const mYorkProfileHtml = engine.playerDetailHtml('m-york-qb', 'rutgers', 'QB');
const mYorkSeasonRibbon = (mYorkProfileHtml.match(/profile-season-ribbon[\s\S]*?<\/section>/) || [''])[0];
check('M. York profile season ribbon prioritizes verified QB production', mYorkSeasonRibbon.includes('<b>1305</b><em>YDS</em>') && mYorkSeasonRibbon.includes('<b>11</b><em>TD</em>') && mYorkSeasonRibbon.includes('<b>N/A</b><em>INT</em>') && mYorkSeasonRibbon.includes('<b>N/A</b><em>SACK</em>') && mYorkSeasonRibbon.indexOf('YDS') < mYorkSeasonRibbon.indexOf('TD'));
...
check('Gameplan snapshot uses separated label-value stat tiles', (coordinatorHtml.match(/home-stat-tile/g) || []).length === 6 && !coordinatorHtml.includes('stat-block') && css.includes('.home-stat-tile span') && css.includes('.home-stat-tile strong'));
check('Gameplan key offensive players use compact horizontal strip', css.includes('scroll-snap-type:x mandatory') && css.includes('.home-player-list::-webkit-scrollbar') && css.includes('flex:0 0 min(284px,86vw)'));
check('Gameplan key-player heading stays Rutgers Offense only', coordinatorHtml.includes('Rutgers Offense') && coordinatorHtml.includes('Key Offensive Players') && !/Rutgers Defense[\s\S]{0,160}Key Offensive Players/i.test(coordinatorHtml));
check('Gameplan home cards clear fixed bottom navigation', coordinatorHtml.includes('home-bottom-spacer') && css.includes('.home-bottom-spacer') && css.includes('132px + env(safe-area-inset-bottom)'));
...
check('Personnel includes current opponent roster hub with sports-table browser', app.includes('function renderOpponent') && opponentRosterHtml.includes('data-roster-hub="opponent"') && opponentRosterHtml.includes('Away Team') && opponentRosterHtml.includes('team-leaders-strip') && opponentRosterHtml.includes('roster-hub-table') && opponentRenderedCount >= PURDUE_OPPONENT_PLAYERS.players.length && new Set((opponentRosterHtml.match(/data-player-name="[^"]+"/g) || [])).size === opponentRenderedCount);
check('Purdue roster hub includes verified LB, DB, and specialist identities', ['A. Warren','B. Leal','R. Daniels','D. Lincoln','K. Smallwood','E. Sawyer','J. Lozano'].every(name => opponentRosterHtml.includes(name)) && (engine.renderOpponent('LB').match(/data-opponent-card/g) || []).length >= 8 && (engine.renderOpponent('DB').match(/data-opponent-card/g) || []).length >= 11 && (engine.renderOpponent('ST').match(/data-opponent-card/g) || []).length >= 1);
const purdueDlRosterHtml = engine.renderOpponent('DL');
check('Purdue roster table uses position-aware verified attributes', purdueDlRosterHtml.includes('<th>PMV</th>') && purdueDlRosterHtml.includes('<th>FMV</th>') && purdueDlRosterHtml.includes('<th>PRC</th>') && purdueDlRosterHtml.includes('<td>85</td>') && !purdueDlRosterHtml.includes('<th>MCV</th>'));
check('Unavailable Purdue offensive groups are reported as source-missing instead of invented', ['QB','RB','WR','TE','OL'].every(group => (engine.renderOpponent(group).match(/data-opponent-card/g) || []).length === 0) && opponentRosterHtml.includes('Source-missing') && opponentRosterHtml.includes('not visible in the current weekly opponent evidence'));
...
check('Roster uses sports-app Home/Away roster hubs instead of one long expanded report', app.includes('function renderTeamRosterHub') && engine.renderTeamRosterHub('team','all').includes('Home Team') && engine.renderTeamRosterHub('opponent','all').includes('Away Team') && css.includes('.team-roster-hub') && css.includes('.roster-hub-table'));
...
check('Stats workspace renders a sports-stat dashboard instead of stacked stat-sheet accordions', statsWorkspaceHtml.includes('stats-dashboard') && statsWorkspaceHtml.includes('sports-stat-table') && statsWorkspaceHtml.includes('stats-leader-strip') && statsWorkspaceHtml.includes('stat-category-grid') && !statsWorkspaceHtml.includes('Team Snapshot') && !statsWorkspaceHtml.includes('Stat Sheet') && !statsWorkspaceHtml.includes('<details'));
check('Stats workspace has phone-safe table metadata and bottom-nav clearance', statsWorkspaceHtml.includes('stat-table-count') && css.includes('.stats-dashboard') && css.includes('padding-bottom:calc(94px + env(safe-area-inset-bottom))') && css.includes('min-width:312px'));
check('Stats workspace hides large empty N/A category cards and shows source-status notes', !statsWorkspaceHtml.includes('stat-table-card empty') && !statsWorkspaceHtml.includes('Purdue Rushing') && statsWorkspaceHtml.includes('stat-source-note') && statsWorkspaceHtml.includes('unavailable categories'));
const purdueTraitFixture = engine.playerDetailHtml('pur-ledg-j-lewis','opponent','DL');
check('Purdue player missing traits render as source-status instead of N/A stacks', purdueTraitFixture.includes('No verified trait data') && purdueTraitFixture.includes('trait-source-note') && !purdueTraitFixture.includes('Development Trait</span><strong>N/A') && !purdueTraitFixture.includes('Physical Abilities</span><strong>N/A'));
const boardOnlyRecruitFixture = engine.recruitDetailHtml('h-scheffler','board');
const verifiedRecruitFixture = engine.recruitDetailHtml('glenn-godwin-qb','prospects');
check('Board-only recruits are flagged as needing verified detail links', boardOnlyRecruitFixture.includes('Needs verified detail link') && boardOnlyRecruitFixture.includes('recruit-link-status'));
check('Verified recruit profile fields populate from linked class/scouting records', verifiedRecruitFixture.includes('National Rank</span><strong>88') && verifiedRecruitFixture.includes('Position Rank</span><strong>6') && verifiedRecruitFixture.includes('Hometown</span><strong>North Brunswick, NJ') && verifiedRecruitFixture.includes('192 lbs') && verifiedRecruitFixture.includes('Dual Threat'));
check('More analytics panels are interactive and data-backed', app.includes('function renderMoreAnalytics') && app.includes('analyticsPanel') && engine.analyticsPanelHtml('team_trends').includes('Team Trends') && engine.analyticsPanelHtml('opponent_tendencies').includes('Opponent Tendencies') && engine.analyticsPanelHtml('recruiting_analytics').includes('Recruiting Analytics'));
check('Portrait presentation uses improved crop and framed source-bound assets', css.includes('object-position:center 18%') && css.includes('radial-gradient(circle at 50% 8%') && css.includes('.source-status-card'));
check('Premium player profile hero uses enlarged fictional portrait crop and safe fact rows', css.includes('width:calc(100% + 28px)') && css.includes('object-position:center 24%') && css.includes('grid-template-columns:minmax(92px,.78fr)') && css.includes('.profile-season-ribbon .sports-stat-strip'));
