...
test("isolated shell uses the direct active package and an allowlist-only startup order", () => {
  ...
  assert.match(fs.readFileSync(f.shell.definitions, "utf8"), /globalThis\.CFB27_APP_BOOT = boot/);
  assert.doesNotMatch(fs.readFileSync(f.shell.definitions, "utf8"), /if \(typeof document !== "undefined"\) boot\(\)/);
});
